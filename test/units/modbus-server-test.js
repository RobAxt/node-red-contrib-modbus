    ...
    it('should initialize node with correct configurations', function (done) {
      ...
      getPort().then((port) => {
        ...
        helper.load(testServerNodes, flow, function () {
          ...
          expect(modbusServer.coilsBufferSize).to.equal(1250)
          expect(modbusServer.holdingBufferSize).to.equal(20000)
          expect(modbusServer.inputBufferSize).to.equal(20000)
          expect(modbusServer.discreteBufferSize).to.equal(1250)
